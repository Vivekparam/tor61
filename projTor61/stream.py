...
class TorStream(object):
	...
	def closeStream(self):
		...
		print "------------------------------closeStream -- client-------"
		# pass

	def closeStreamAndSendRelayEnd(self):
		zeroes = 0x0000
		cell = struct.pack('>HBHHIHB' + ('x' * TorStream.PADDING_RELAY) , self.c_id, TorStream.CELL_TYPE_RELAY, self.streamNum, 
								zeroes, TorStream.RELAY_DIGEST_CONSTANT, 0, TorStream.RELAY_CMD_END)
		self.bufferToRouter.put(cell)
		time.sleep(1)
		self.state = TorStream.State.stopped
		print "------------------------------closeStream -- server (sending RELAY_END)-------"
		# pass
